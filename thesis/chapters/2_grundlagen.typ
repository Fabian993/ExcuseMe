...
#figure(
  ...
  // author = {Jan Schubert},
  // date = {12.05.2026},
)<abb:Client-Server-Architekturprinzip>

Häufig wird bei der Entwicklung von Software die Client-Server-Architektur angewandt.
Diese unterteilt - wie in @abb:Client-Server-Architekturprinzip ersichtlich - zwei miteinander kommunizierende Systeme in Client und Server. Der Server stellt die zentrale Logik der Anwendung dar und wertet die Anfragen des Clients aus. Dafür einigen sich beide auf ein bestimmtes Protokoll, das definiert, wie die Kommunikation stattfindet.
@client_server
...
@abb:Model-View-Controller stellt das @MVC:l Design Pattern dar. Dieses Architekturprinzip trennt Datenhaltung (Model), Nutzeroberfläche (View) und Steuerlogik (Controller) voneinander, um eine bessere Arbeitsteilung im Team zu erzielen und die Wartbarkeit des Gesamtsystems zu verbessern.
@mvc_pattern
...
Dabei gibt es verschiedene Vorgehensweisen, um seine Ziele zu erreichen. Viele von diesen Entwicklungsmodellen kommen dabei aus der Projektplanung. Ihre Aufgabe ist es, die verschiedenen Tätigkeiten der Entwickler im Lebenszyklus der Software zu strukturieren. Die Modelle werden in mehrere Gruppen unterteilt: Von formell bis informell, was vom geschäftlichen Bezug zum Kunden abhängt, sowie linear bis iterativ, was wiederum von der konkreten Vorgehensweise des Modells selbst abhängt. 
@modelle_swe
...
Das Wasserfallmodell ordnet die Tätigkeiten in Analyse, Design, Programmierung, Test, Installation und Instandhaltung. Die einzelnen Schritte werden hierbei genau mitgeschrieben und festgehalten. Der große Nachteil dabei ist jedoch, dass jede Stufe vom Abschluss der vorherigen abhängt. Fehler wirken sich also auf die jeweils nächste Phase aus. Auch die Anforderungen sollten bereits beim ersten Schritt, der Analyse, möglichst genau bekannt sein, weil nur so ein genauer, schrittweiser Plan erstellt werden kann.
@modelle_swe

Scrum unterscheidet sich als agile Entwicklungsmethode grundlegend: \
Alle zwei bis vier Wochen findet ein Sprint Review statt, in dem Aufträge, die bis zum nächsten Meeting umgesetzt werden sollten, geplant werden. Dazu kommt noch die Sprint-Retrospektive, wo der neue Fortschritt kontrolliert und gegebenenfalls eine Korrektur aufgetragen wird. Kommt es zur Änderung der Anforderungen, können diese bereits in der nächsten Planung berücksichtigt werden, weshalb man hier von "agiler Entwicklung" spricht.
@modelle_swe
...
Das @ORM im Hintergrund nennt sich Django, das es ermöglicht Datenbankzugriffe über Python-Modelle umzusetzen, anstatt SQL-Abfragen schreiben zu müssen. Dabei beschreibt ein Django-Modell die Struktur der gespeicherten Daten und entspricht in der Regel einer Datenbanktabelle. Auf dieser Grundlage stellt Django automatisch eine Datenbank-@API:s bereit, mit der Objekte erstellt, abgerufen, verändert und gelöscht werden können. Dadurch bleibt der Code übersichtlicher und stark Python orientiert, während viele Datenbankzugriffe über das Framework laufen.
