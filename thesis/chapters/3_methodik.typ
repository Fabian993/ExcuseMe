...
Die nachstehende Tabelle gibt eine Übersicht über die zentralen API-Endpunkte und deren typische JSON-Responses. Für Listenendpunkte wird dabei die von Django REST Framework verwendete Wrapper-Struktur mit `count`, `next`, `previous` und `results` verwendet. \
...
In ExcuseMe müssen Benutzer, Rollen, Abwesenheiten und weitere zugehörige Informationen konsistent gespeichert und miteinander vereknüpft werden. Dafür sind relationale Datenbanken besonders geeignet, da sie mit Tabellen, Schlüsseln und Transaktionen eine hohe Datenintegrität gewährleisten.
...
=== JSON Web Token
//Warum JWT?
@JWT ist ein Standard zur sicheren Übetragung von Informationen zwischen Client und Server und wird in Webanwendungen zur Authentifizierung eingesetzt. Nach dem Login erhält der Client einen *Token*, der bei weiteren Anfragen
